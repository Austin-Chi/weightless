...
simd::float4x4 mtlm::view_matrix(simd::float3 right, simd::float3 up, simd::float3 forward, simd::float3 position)
{
    simd_float4 col0 = {right[0], up[0], forward[0], 0.0f};
    simd_float4 col1 = {right[1], up[1], forward[1], 0.0f};
    simd_float4 col2 = {right[2], up[2], forward[2], 0.0f};
    simd_float4 col3 = {-simd::dot(right, position), -simd::dot(up, position), -simd::dot(forward, position), 1.0f};
